...
def M_high(start_date, end_date, N=10, window_size=20):
    ...
    def calc_rolling_top_n_sum(group):
        ...
        factor_values = np.sum(top_ret, axis=1)
...
def M_low(start_date, end_date, N=10, window_size=20):
    lookback_start = (pd.to_datetime(start_date) - pd.Timedelta(days=40)).strftime('%Y-%m-%d')
    ...
    def calc_rolling_top_n_sum(group):
        ...
        t_windows = sliding_window_view(turnover, window_shape=window_size) # 注意: 前 w-1 个不够window_size 不会创建
        r_windows = sliding_window_view(ret, window_shape=window_size)
        top_indices = np.argsort(t_windows, axis=1)[:, :N]
        top_ret = np.take_along_axis(r_windows, top_indices, axis=1)
        factor_values = np.sum(top_ret, axis=1)
    ...
    log("======= 计算因子 M_low =======")
    m_low = df_return.groupby('order_book_id', group_keys=False).apply(calc_rolling_top_n_sum) # group_keys=False 禁止 Pandas 在结果索引中自动添加分组键 (order_book_id) 否则变成三层索引 [obi, obi, date]
    m_low = pd.DataFrame(m_low.loc[(m_low.index.get_level_values('date') >= start_date) & (m_low.index.get_level_values('date') <= end_date)])
    m_low.columns = ['M_low']
    return m_low

def W_reverse(start_date, end_date, N=10, window_size=20):
    high = M_high(start_date, end_date)
    low = M_low(start_date, end_date)
    high['M_low'] = low['M_low']
    high['W_reverse'] = high['M_high'] - high['M_low']
    return high

if __name__ == '__main__':
    # W = W_reverse('2016-01-01', '2025-12-31')
    # W.to_parquet('W.parquet')
    period = 1
    df_factor = pd.read_parquet('W.parquet')
